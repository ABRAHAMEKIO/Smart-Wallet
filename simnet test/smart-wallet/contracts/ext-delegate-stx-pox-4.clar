    ...
    (let 
        ...
        (if (is-eq "delegate" (get action details))
            (begin
                ...
                (try! (as-uint-response (as-contract (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.pox-4 delegate-stx
            ...
            (if (is-eq "revoke" (get action details))
                (begin
                    (try! (match (as-contract (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.pox-4 revoke-delegate-stx))
                        success (ok success)

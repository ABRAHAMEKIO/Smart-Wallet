...
import { ClarityType, Cl, uintCV, ResponseOkCV, ResponseErrorCV, standardPrincipalCV, noneCV, contractPrincipalCV, bufferCV, boolCV } from '@stacks/transactions';
...
const memoCV = noneCV();
const sip010Contract = contractPrincipalCV('SP3FBR2AGK5H9QBDH3EEN6DF8EK8JY7RX8QJ5SVTE', 'sip-010-trait-ft-standard.sip-010-trait');
const sip009Contract = contractPrincipalCV('SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9', 'nft-trait.nft-trait');
const extensionContract = contractPrincipalCV('SP32AEEF6WW5Y0NMJ1S8SBSZDAY8R5J32NBZFPKKZ', 'nope');
...
describe('test `smart-wallet-standard` public functions', () => {
  ...
  //   const extensionResponse = await simnet.callPublicFn("smart-wallet", "extension-call", [extensionContract, payload], address1);
  //   console.log('Extension Call Response:', Cl.prettyPrint(extensionResponse.result));
  ...
  it('transfers 1 Nft to wallet', async () => {
    ...
    const sip9transfer = await simnet.callPublicFn("smart-wallet", "sip009-transfer", [NftId, recipientCV, sip009Contract], address1);

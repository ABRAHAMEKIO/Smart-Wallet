...
const Extensions = ({ clientConfig, contractState, setConfirmationModal, setTx }) => {
    return (
        <div>
            <Accordion variant="splitted">
                <AccordionItem key="1" aria-label="Accordion 1" title={<label className='flex gap-4 items-center'><PiLockKey size="20px" className='text-warning' /> Delegate Stx</label>}>
                    <DelegateStxPox4 clientConfig={clientConfig} contractState={contractState} setConfirmationModal={setConfirmationModal} setTx={setTx} />
                </AccordionItem>
            </Accordion>
        </div>
    );

...
function DepositModal({ clientConfig, openDepositModal, closeDepositModal }) {
  ...
  async function depoFt() {
    ...
    const condition01 = Pc.principal(authedUser)
      .willSendLte(sendAmount)
      .ft(
        selectedFt.contract_principal,
        selectedFt.contract_identity.split("::")[1]
      );
  ...
  async function selectNft(asset) {
    ...
    let nftHoldings = (
      await axios.get(
        `${api[activeNetwork]}/extended/v1/tokens/nft/holdings?principal=${nftOwner}&asset_identifiers=${contract_address}::${name}&limit=${count}&offset=0`
      )
    ).data.results;
    nftHoldings = await Promise.all(
      nftHoldings.map(async (res) => {
        const {
          asset_identifier,
          tx_id,
          value: { repr },
  ...
  async function depoNft(asset) {
    ...
    const condition01 = makeStandardNonFungiblePostCondition(
      authedUser,
      postConditionCode,
  ...
  return (
    <BaseModal
      baseModalsOpen={openDepositModal}
      baseModalOnClose={closeDepositModal}
    >
                  ...
                  <div className="flex gap-3 justify">
                    ...
                    <div className="flex flex-col gap-1 items-start justify-center">
                      <h4 className="text-small font-semibold leading-none text-default-600">
                        Balance:{" "}
                        <code className="text-warning">{stx.balance}</code>
                      </h4>
                    ...
                    <Select
                      ...
                      startContent={
                        <Avatar
                          size="sm"
                          src={
                            selectedFt.image_uri || selectedFt.placeholder_icon
                          }
                        />
                      }
                    >
                      {fungible_Tokens.map(
                        ({
                          ...
                          <SelectItem
                            startContent={
                              <Avatar src={image_uri || placeholder_icon} />
                            }
                            endContent={
                    ...
                    <Select label="Assets List">
                      {non_Fungible_Tokens.map(
                        ({
                          ...
                          <SelectItem
                            startContent={<Avatar src={image_url} />}
                            endContent={
                              isDisabled && name === targetName ? (
                                <Spinner color="warning" />
                              ) : (
                          ...
                          <div className="w-full flex justify-between justify-center items-center">
                            ...
                            <div className="flex flex-col gap-2">
                              <Button
                                ...
                                size="sm"
                                onPress={() =>
                                  depoNft({ asset_id, asset_identifier })
                                }
                              >
                                <IoMdSend />
                              </Button>
